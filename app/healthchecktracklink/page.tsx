...
export default function HealthCheckTracklink() {
          ...
          {/* NAVBAR */}
              <nav className="bg-blue-900 text-white px-4 py-2 flex items-center justify-between">
                      <div className="flex gap-6 font-semibold">
                                <button onClick={() => router.push("/")} className="hover:text-yellow-300">Renovaciones</button>button>
                                <button onClick={() => router.push("/")} className="hover:text-yellow-300">Instalaciones</button>button>
                                <button className="text-yellow-300 underline">HealthCheck</button>button>
                                <button onClick={() => router.push("/")} className="hover:text-yellow-300">HealthCheck Mazda</button>button>
                                <button onClick={() => router.push("/")} className="hover:text-yellow-300">Salir</button>button>
                      </div>div>
                      <div className="flex items-center gap-4">
                                <span className="text-xl font-bold tracking-widest">TRACKLINK<span className="text-yellow-400">«««</span>span></span>span>
                                <button className="bg-gray-200 text-blue-900 text-xs px-3 py-1 rounded hover:bg-white">API actualizar</button>button>
                      </div>div>
              </nav>nav>
              ...
              <div className="p-4">
                {/* ENCABEZADO */}
                      <div className="flex items-center justify-between mb-3">
                                <div className="flex items-center gap-3">
                                            <span className="text-blue-900 font-bold text-lg">HealthCheck Tracklink</span>span>
                                  {cargando && <span className="text-gray-500 text-xs">Cargando datos...</span>span>}
                                  {!cargando && !error && (
                        <span className="text-gray-500 text-xs">{datosFiltrados.length} de {datos.length} registros</span>span>
                                            )}
                                </div>div>
                          ...
                          {filtrosActivos.map(col => {
                                            ...
                                            return (
                                                                  <span key={col} className="bg-blue-100 border border-blue-300 text-blue-800 text-xs px-2 py-0.5 rounded flex items-center gap-1">
                                                                                        <strong>{colDef?.label || col}:</strong>strong> {filtros[col]}
                                                                                        <button onClick={() => limpiarFiltro(col)} className="ml-1 text-blue-600 hover:text-red-600 font-bold">×</button>button>
                                                                  </span>span>
                                                                );
                        })}
                                        <button onClick={limpiarTodosFiltros} className="text-xs text-red-600 hover:underline ml-1">Limpiar todo</button>button>
                        </div>div>
                                            )}
                                            <button
                                                            onClick={cargarDatos}
                ...
                {error && <div className="text-red-600 text-xs mb-2">{error}</div>div>}
                            ...
                            return (
                                                  <th key={col.key} className="border border-blue-700 px-2 py-1 whitespace-nowrap text-left relative">
                                                                        <div className="flex items-center gap-1">
                                                                                                <span className="flex-1">{col.label}</span>span>
                                                                                                <button
                                                                                                                            onClick={(e) => {
                                                                                                                                                          e.stopPropagation();
                                                    ...
                                                    {estaAbierto && (
                                                                            <div className="absolute top-full left-0 z-50 bg-white border border-gray-300 shadow-lg min-w-[200px] max-h-[300px] flex flex-col" style={{marginTop: "2px"}}>
                                                                                                      ...
                                                                                                      <div className="overflow-y-auto flex-1">
                                                                                                        {tieneFiltr && (
                                                                                                            <div
                                                                                                                                              className="px-3 py-1.5 hover:bg-blue-50 cursor-pointer text-gray-700 font-medium border-b border-gray-100 text-red-600"
                                                                                                        ...
                                                                                                        {valoresUnicos(col.key as string).slice(0, 200).map(valor => (
                                                                                                            <div
                                                                                                                                            ...
                                                                                                                                            <span className="truncate">{valor || "(vacío)"}</span>span>
                                                                                                              </div>div>
                                                                                                          ))}
                                                                                                        </div>div>
                                                                              </div>div>
                                                                                                                                                ...
                                                                                                                                                <td key={col.key} className={className}>
                                                                                                                                                  {col.key === "Serv. Hasta" ? (
                                                                                                                                                                                <span className="flex items-center gap-1">
                                                                                                                                                                                                                {String(val ?? "")}
                                                                                                                                                                                                                {vencido && <span className="bg-red-500 text-white text-xs px-1 rounded ml-1">Vencido</span>span>}
                                                                                                                                                                                                                {porVencer && !vencido && <span className="bg-yellow-400 text-black text-xs px-1 rounded ml-1">Por vencer</span>span>}
                                                                                                                                                                                                              </span>span>
                                                                                                                                                                              ) : (
                                                                                                                                                                                String(val ?? "")
                                                                                                                                                                              )}
